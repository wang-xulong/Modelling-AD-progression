function [ perform_mat,best_params] = progressModelCrossValidation1Param...
...
%   X:             input data
%   Y:             output data
%   obj_func_str:  1-parameter optimization algorithms mat_rmse_std_t_TGL
...
%         W = obj_func(cv_Xtr, cv_Ytr, param_range(p_idx),param_range(p_idx),param_range(p_idx), obj_func_opts);
        ...
        for p_id_1=1:length(param_range)
            for p_id_2=1:length(param_range)
                for p_id_3=1:length(param_range)
                    W = obj_func(cv_Xtr, cv_Ytr, param_range(p_id_1),param_range(p_id_2),param_range(p_id_3), obj_func_opts);
                    temp=eval_func(cv_Yte, cv_Xte, W);
                    if(max_eval<temp)
